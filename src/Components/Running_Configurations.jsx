...
const Running_Configurations = () => {
  const [runningconfig, setRrunningconfig] = useState();
  ...
  const fetchData = async () => {
    {
      iP == null
        ...
        : await fetch(
            `http://localhost:3000/dashboard/monitoring/running?selectedDeviceIP=${iP}`
          )
            .then((res) => res.json())
            .then((data) => {
              setRrunningconfig(data.RunningConfig);
  ...
  const fetchData2 = async () => {
    {
      runningconfig == null
        ? notifyD("Please Select a Device")
        : await fetch(
            `http://localhost:3000/dashboard/monitoring/save?content=${runningconfig}`
          )
            .then((res) => res.json())
            .then((data) => {
  ...
  return (
    <div className="flex flex-col gap-5 bg-gray-300 rounded-2xl p-5 w-full shadow-lg shadow-black ">
      <div className="font-bold text-xl">Running Configurations </div>
...
export default Running_Configurations;

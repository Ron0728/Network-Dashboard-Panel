...
const Manual_IP_Route = () => {
  ...
  const fetchData = async () => {
    {
      (iP && selectedInterfaces) == null
        ? notifyD(
            "Please Select a Device and Interface then Enter IP & Subnet "
          )
        : await fetch(
            `http://localhost:3000/dashboard/troubleshooting/manualroute?selectedDeviceIP=${iP}&&selectedDeviceInt=${selectedInterfaces}&&EnteredIP=${enterdIP}&&EnteredSub=${enterdSubnet}`
          )
            .then((res) => res.json())
            .then((data) => {
              setRouteMSG(data.message);
              notifyG(routeMSG);
  ...
  return (
    <div className="flex flex-col bg-gray-300 rounded-2xl gap-5 p-5 w-full h-full shadow-lg shadow-black ">
      <div className="font-bold text-xl">Manual IP Route</div>
